...
const UploadClass = () => {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const response = await teacherAxiosInstance.get("/getCategories");
  ...
  const handleSubmit = async (event) => {
    ...
    const discountPriceValue = parseFloat(formData.discountPrice);

    if (isNaN(priceValue) || isNaN(discountPriceValue)) {
  ...
  return (
    <div className="bg-dashboard-bg h-screen">
      <hr />
      <div className="h-auto   bg-dashboard-bg">
