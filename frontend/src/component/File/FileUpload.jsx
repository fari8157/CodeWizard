  ...
  const handleDrop = (e) => {
    ...
    if(selectedFile && selectedFile.size <= 1048576){ // Check size 1 MB (1,048,576 bytes)
      ...
      if(fileName.endsWith('.doc') || fileName.endsWith('.docx') || fileName.endsWith('.pdf')){
        handleFileSelect(inputName, files);
        setFileName(selectedFile.name);
      }else{
        toast.error("please choose document file", { position: "bottom-center", duration: 2000, style: { background: '#FFC107', color: 'red' }, icon: '⚠️' });
      }
    }else{
      toast.error("please choose file below 1 MB size", { position: "bottom-center", duration: 2000, style: { background: '#FFC107', color: 'red' }, icon: '⚠️' });
    }
  };
  ...
  const handleChange = (e) => {
    ...
    if (file && file.size <= 1048576){ // Check size 1 MB (1,048,576 bytes)
      ...
      if (allowedFileTypes.includes(file.type)) {
        ...
        toast.error("Please upload pdf or docx file", { position: "bottom-center", duration: 2000, style: { background: '#FFC107', color: 'red' }, icon: '⚠️' });
      }
    }else{
      toast.error("please choose file below 1 MB size", { position: "bottom-center", duration: 2000, style: { background: '#FFC107', color: 'red' }, icon: '⚠️' });
    }
